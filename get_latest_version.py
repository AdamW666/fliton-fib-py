...
def get_latest_version_number() -> str:
    req = requests.get("https://pypi.org/pypi/fliton-fib-py/json")
...
def write_version_to_file(version_number: str) -> None:
    version_file_path: str = str(pathlib.Path(
        __file__).parent.absolute()) + "/fliton_fib_py/version.py"
...
if __name__ == '__main__':
    write_version_to_file(pack_version_number(increase_version_number(
        unpack_version_number(get_latest_version_number()))))
